...
export function ProductSections({ onApiSetup }: { onApiSetup: () => void }) {
  return <div className="product-content">
    ...
    <section id="about" className="content-section about-section"><div className="content-intro"><p className="eyebrow">ABOUT ZHUDEJIN</p><h2>地图算路线，我们帮助你选房</h2><p>租房平台告诉你有哪些房子，高德或百度告诉你一条路线怎么走；住得近把两类信息组合起来，回答“哪些房子值得优先看”。</p></div><div className="about-grid"><article><Home /><h3>不是租房交易平台</h3><p>不提供支付和签约，只保留合法房源来源链接，方便回到来源平台核验。</p></article><article><WalletCards /><h3>减少重复比较</h3><p>不用在租房平台和地图之间反复复制几十次地址，一次设置即可统一比较。</p></article><article><KeyRoundIcon /><h3>可接入真实 API</h3><p>演示版使用公开底图和本地房源；配置高德与 Supabase 后切换为真实服务。</p><button onClick={onApiSetup}>打开 API 配置</button></article></div></section>
  </div>
}
