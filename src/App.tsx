...
function App() {
  const showDeveloperApiSetup = import.meta.env.DEV
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark"><Home size={18} /></div><div><strong>住得近</strong><span>WORK TO HOME</span></div></div><nav className="top-nav"><a className="active" href="#explore">工作找房</a><a href="#how">推荐逻辑</a><a href="#about">产品说明</a></nav><div className="top-actions">{showDeveloperApiSetup && <button className="api-config-button" onClick={() => setApiSetupOpen(true)}><KeyRound size={14} /> API 配置</button>}<span className={`mode-badge ${resultMode}`}>{resultMode === 'production' ? '真实数据' : resultMode === 'staging' ? '内测数据' : '本地演示'}</span><span className="saved-summary"><Heart size={15} fill="currentColor" /> {favoriteIds.length} 已收藏</span><button className="avatar-button" aria-label="菜单" aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen((open) => !open)}><Menu size={19} /></button></div>{mobileMenuOpen && <nav className="mobile-nav"><a href="#explore" onClick={() => setMobileMenuOpen(false)}>工作找房</a><a href="#how" onClick={() => setMobileMenuOpen(false)}>推荐逻辑</a><a href="#about" onClick={() => setMobileMenuOpen(false)}>产品说明</a>{showDeveloperApiSetup && <button onClick={() => { setMobileMenuOpen(false); setApiSetupOpen(true) }}><KeyRound size={14} /> API 配置</button>}</nav>}</header>
    ...
    <ProductSections onApiSetup={showDeveloperApiSetup ? () => setApiSetupOpen(true) : undefined} />
    ...
    {compareOpen && <ComparePanel items={compareItems} onClose={() => setCompareOpen(false)} onRemove={toggleCompare} />}{showDeveloperApiSetup && apiSetupOpen && <ApiSetupPanel onClose={() => setApiSetupOpen(false)} />}{toast && <div className="toast"><Check size={15} /> {toast}</div>}
  </div>
}
